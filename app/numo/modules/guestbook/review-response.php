...
?>
	<form method="post">
	<table class="messages_received" cellpadding="0" cellspacing="0" width='600'>
	<?php
	if($_GET['display'] != "response_only") {
	?>
	<tr class="button_row">
	...
	<tr><td colspan="2" style="border: 1px solid #fff; border-bottom: 1px solid #bbb;">&nbsp;</td></tr>
	<tr><th colspan="2">Response Copy</th></tr>
	<tr><td colspan="2">
		<table class="reponse_table" cellpadding="0" cellspacing="5" width='100%'>
		<?php
		...
		while($field = mysql_fetch_array($results)) {
			...
			if($field['input_type'] == "heading") {
				...
				if ($field['input_type'] == "text") {
					print '<tr><th>'.$field['name'].':</th><td><input style="width: 99%;" name="slot_'.$field['slot'].'" value="'.nl2br($fieldValue).'" /></td></tr>';
				
				} else if ($field['input_type'] == "textarea") {
				print '<tr><th>'.$field['name'].':</th><td><textarea style="width: 99%; height: 100px; " name="slot_'.$field['slot'].'">'.stripslashes($fieldValue).'</textarea></td></tr>';
