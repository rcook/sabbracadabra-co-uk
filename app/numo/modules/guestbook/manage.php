...
<ul class="breadcrumb">
  ...
  <li>&nbsp; <a href="module/guestbook/edit/" style='margin-top: -2px;' class='btn btn-success btn-mini'>Create New Guestbook</a></li>
</ul>
...
echo '<table class="table table-striped"><tr><th style="width: 50px; text-align: center;">Public</th><th class="highlight_label">Title</th><th>&nbsp</th></tr>';

while($row = mysql_fetch_array($results)) {
	echo '<tr class="'.($oddEvenCounter % 2 == 0 ? 'even' : 'odd').'"><td style="text-align: center;"><img src="images/'.(($row['restrict_access'] == "1") ? 'no' : "yes").'.gif" />'.
			 '</td><td class="group_name">'.$row['name'].'</td><td style="text-align: right"><a class="btn btn-primary" href="module/'.$_GET['m'].'/edit/?id='.$row['id'].'">Manage</a> <a class="btn" href="module/'.$_GET['m'].'/components/?id='.$row['id'].'">Get Component Code</a> <a class="btn btn-danger" href="module/'.$_GET['m'].'/'.$_GET['i'].'/" onclick="return confirmRemove(\''.$row['id'].'\');">Remove</a></td></tr>';
