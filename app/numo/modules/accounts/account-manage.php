...
<form method="post" name='main_submit' id='main_submit' >
  ...
  <input type='hidden' id='order_by' name='order_by' value='<?=$orderBy?>' />
	...
	<ul class="form_display">
		...
		<li><label for="results_per_page">Results Per Page:</label>
...
while($row = mysql_fetch_array($results)) {
	...
	echo '<td><form method="post"><input type="hidden" id="page" name="page" value="'.$currentPage.'" /><input type="hidden" name="results_per_page" value="'.$resultsPerPage.'" /><select ';
	...
	echo '<td>';
	if (($row['is_admin'] == '1' && $_SESSION['is_admin'] == '1') || ($row['is_admin'] != 1 && $access->hasAccess("accounts", "account-edit"))) {
		echo '<a href="module/'.$_GET['m'].'/account-edit/?id='.$row['id'].'">Edit</a> ';
	    echo '<a href="module/'.$_GET['m'].'/'.$_GET['i'].'/" onclick="return confirmRemove(\''.$row['id'].'\', \''.$row['is_admin'].'\');">Remove</a>';
	}
	echo '</td>';
	echo '</tr>';
...
<div style='text-align: center; padding-top: 3px;'>Displaying results <?=($offset + 1)?> to <?=($offset+$resultsThisPage)?> of <?=$totalResults?><br/>
<?php
if ($currentPage < 5) {
  $pageNavStart = 1;
...
for ($page = $pageNavStart; $page <= $totalPages && $page < ($pageNavStart+$totalPagesToDisplay); $page++) {
  print "<input onclick='jumpToResultsPage(this)' ";
  if ($page == $currentPage) {
    print "class='page-button-selected' ";
  } else {
    print "class='page-buttons' ";
  }
  print "type='button' value='{$page}' />";
}
?>
</div></div>
...
function jumpToResultsPage(button) {
  document.forms['main_submit'].page.value = button.value;
  document.forms['main_submit'].submit();
}
