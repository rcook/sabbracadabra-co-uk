...
<?php } ?>	
	...
	newHTML = newHTML + '<li><a href="javascript:manageCategory(\''+divIdName+'\')">Manage</a><a href="javascript:removeCategory(\''+divIdName+'\')">Remove</a></li></ul>';
...
<?php } ?>
				<li><a href="javascript:manageCategory('<?=$category['id']?>')">Manage</a><a href="javascript:removeCategory('<?=$category['id']?>')">Remove</a></li>
			</ul>
		</div>
	<?php
	}
	...
	if($parentId != 0) {
	?><input type="button" name="nocmd2" value="Back" onClick="returnToParentCategory()" /><?php } ?><input type="button" name="nocmd2" value="Add New Category" onClick="addItem()" />
