...
?>
	...
	if(mysql_num_rows($results) > 0) {
		echo '<table class="table table-striped"><tr><th style="width: 20px;">&nbsp;</th><th style="width: 40px;text-align: center">ID</th><th style="width: 40px;">Shipped</th><th style="width: 100px">Date</th><th class="highlight_label">Customer</th><th>Method</th><th style="width: 50px">Total</th><th>&nbsp</th></tr>';
 ...
 <br/>
	...
	if(mysql_num_rows($results) > 0) {
		echo '<table class="table table-striped"><tr><th style="width: 20px;">&nbsp;</th><th style="width: 40px; text-align: center;">ID</td><th style="width: 40px">Shipped</th><th style="width: 100px">Date</th><th class="highlight_label">Customer</th><th>Method</th><th style="width: 50px">Total</th><th>&nbsp</th></tr>';
	...
	if(mysql_num_rows($results) > 0) {
		echo '<table class="table_data_layout"><tr><th style="width: 20px;">&nbsp;</th><th style="width: 40px;">ID</th><th style="width: 100px">Date</th><th class="highlight_label">User</th><th>Method</th><th>Total</th><th>&nbsp</th></tr>';
        $orderTotal = 0;
		while($row = mysql_fetch_array($results)) {
...
<p>"Abandoned Carts" are shopping carts which were abanadoned before the user proceeded to checkout.</p>
	...
	if(mysql_num_rows($results) > 0) {
		echo '<table class="table table-striped"><tr><th>&nbsp;</th><th>ID</th><th>Date</th><th class="highlight_label">User</th><th>Amount</th><th>&nbsp</th></tr>';
        $orderTotal = 0;
		while($row = mysql_fetch_array($results)) {
			$orderTotal = 0;
			$sql = "SELECT i.`id`, i.`quantity`, i.`item_cost`, p.`slot_1` FROM `shopping_cart_order_items` i, `shopping_cart_products` p WHERE i.`order_id`='{$row['id']}' AND i.`product_id`=p.`id` AND p.`site_id`='".NUMO_SITE_ID."'  ORDER BY i.`id` asc";
			...
			echo '<tr class="'.($oddEvenCounter % 2 == 0 ? 'even' : 'odd').'"><td><img src="modules/shopping_cart/images/'.$displayStatusImageSrc.'" alt="'.$displayStatusImageDesc.'" title="'.$displayStatusImageDesc.'" /></td><td>'.$row['id'].'</td><td>'.($row['c_date'] == "" ? "Unknown" : $row['c_date']).'</td><td>'.($row['first_name'].$row['last_name'] == "" ? "Guest" : $row['first_name'].' '.$row['last_name']).'</td><td>'.number_format($orderTotal, 2, '.', ',').'</td><td style="text-align: right;  width: 150px;"><a class="btn" href="module/'.$_GET['m'].'/review-order/?id='.$row['id'].'">Review</a> <a class="btn btn-danger" href="module/'.$_GET['m'].'/customer-orders/?cmd=delete&id='.$row['id'].'" onclick="return confirm(\'Are you sure you want to permanently delete this order from the system?\')">Delete</a></td></tr>';
