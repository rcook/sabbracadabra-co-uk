...
$layoutFilePath = "modules/newsletter/layouts/".$_GET['layout'].".htm";
...
if($_POST['cmd'] == "Cancel") {
	header('Location: '.NUMO_FOLDER_PATH.'module/'.$_GET['m'].'/configure/');
...
ul.styleProperties li label {padding-left: 5px;}
...
<h2>Configure: <?=str_replace("_"," ",$_GET['layout'])?> Layout</h2>
<p>Below is a preview of the newsletter layout.  You will be able to re-configure the default background and text colors for each the sections of the layout by clicking in the layout section.  When you click on a layout section its current color settings will appear under the 'Default Colors' heading.</p>
<table height="100%">
<tr>
	<td valign="top" width="210px" height="200px">
		<h2>Default Colors</h2>
		<p>Click on a section in the layout to the right to change its default color options.</p>
		<?php
			$styleArr = array();
			...
			foreach($styleArr as $name => $styles) {
				...
				print "<li><h3>".ucwords(str_replace("_"," ",str_replace("stylerclass_numo_newsletter_","",trim($name))))."</h3></li>\n";
	...
	<td valign="top">
	<iframe id="layout" src="<?=$layoutFilePath."?t=".time()?>" width="700px" height="100%" frameborder="0"></iframe>
	</td>	
</tr>
</table>
