...
?>
	...
	if($_POST['cmd'] == "update") {
		...
		?>
			<p>No subscription lists available</p>
            <?php if (!$bootstrapStyling) { ?></td></tr></table><? } else { ?></div></div><? } ?>
		<?php
			return;
		}
		?>
		...
		?>
			<li><input class='pull-left' style='margin-right: 10px !important;' type="checkbox" name="lists[]" checked="checked" id="numo_newsletter_list_<?=$row['id']?>" value="<?=$row['id']?>" /><label class="checkbox_label" for="numo_newsletter_list_<?=$row['id']?>"><?=$row['name']?></label></li>
		<?php
			}	else {
		?>
			<li><input class='pull-left' style='margin-right: 10px !important;' type="checkbox" name="lists[]" id="numo_newsletter_list_<?=$row['id']?>" value="<?=$row['id']?>" /><label class="checkbox_label" for="numo_newsletter_list_<?=$row['id']?>"><?=$row['name']?></label></li>
		<?php
			}
		...
		<input class="btn btn-primary" type="submit" name="nocmd" value="<?=NUMO_SYNTAX_NEWSLETTER_ACCOUNT_SUBSCRIBE_BUTTON?>" />
...
<?php
	...
	if($_POST['cmd'] == "unsubscribe") {
		...
		print NUMO_SYNTAX_NEWSLETTER_PROCESSED_UNSUBSCRIBE_REQUEST."";
		if (!$bootstrapStyling) { ?></td></tr></table><? } else { ?></div></div><? }
		...
		if($_POST['name'] == "") {
			...
			if($row = mysql_fetch_array($result)) {
				...
				if($_POST['group_id'] == "" || $_POST['group_id'] == "0") {
					print "<p class='error'>Subscription request could not be processed.  A default account group must be selected before a new subscribers can be created.</p>";
								 if (!$bootstrapStyling) { ?></td></tr></table><? } else { ?></div></div><? }
...
?>
	<form method="post">
	<ul class="numo_newsletter_subscription_component" style='list-style-type: none;'>
		...
		<li>&nbsp;</li>
		<li><input class='btn btn-primary' type="submit" name="nocmd" value="<?=NUMO_SYNTAX_NEWSLETTER_VISITOR_SUBSCRIBE_BUTTON?>" /><input class='btn' type="button" name="nocmd" value="<?=NUMO_SYNTAX_NEWSLETTER_VISITOR_UNSUBSCRIBE_BUTTON?>" onclick="numoNewsletter_unsubscribe(this.form)" /></li>
	</ul>
...
if (!$bootstrapStyling) { ?></td></tr></table><?php } else { ?></div></div><?php }
?>