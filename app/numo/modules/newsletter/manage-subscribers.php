...
<ul class="breadcrumb">
  ...
  <li>&nbsp; <a href="module/newsletter/create-subscriber/" style='margin-top: -2px;' class='btn btn-success btn-mini'>Create Subscriber</a></li>
</ul>
...
<form method="post">
	...
	<ul class="form_display">
		...
		<li><label for="submit_input">&nbsp;</label><input type="submit" name="nocmd" id="submit_input"  class='btn btn-primary' value="Search" /></li>
	</ul>
	</fieldset>
...
<form style='padding-bottom: 10px' action="module/<?=$_GET['m']?>/create-subscriber/" method="get">
  <input type="submit" class='btn btn-success' value="Create New Subscriber" />
</form>
-->
<table class="table table-striped"><tr><th class="highlight_label">Name</th><th>Email</th><th style='text-align: center; padding-left: 20px;'><form style='margin: 0px;' action="module/<?=$_GET['m']?>/manage-subscribers/?display=response_only" method="post" target="export-frame">
  <input type="submit" class='btn pull-right ' value="Export to CSV" />
...
?>
<tr><td><?=$row['slot_4']?></td><td><?=$row['slot_3']?></td><td><a class='pull-right btn btn-info' href="module/<?=$_GET['m']?>/edit-subscriber/?id=<?=$row['id']?>">Edit</a></td></tr>
<?
}
?>
</table>
