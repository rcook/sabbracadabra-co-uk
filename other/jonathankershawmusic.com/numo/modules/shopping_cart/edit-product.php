...
if($_POST['cmd'] == "update") {
  foreach ($_POST as $key => $value) {
	...
	foreach($fieldOrderArr as $key => $id) {
		...
		//update query
		$sql = "UPDATE `shopping_cart_optional_product_attributes` SET position='".$position."',type='".$_POST[$id.'__type']."',label='".htmlentities($_POST[$id.'__name'])."',required='".htmlentities($_POST[$id.'__required'])."',label_fg_color='".htmlentities($_POST[$id.'__label_fg_color'])."' WHERE id='".$idNum."'";
		//print $sql."<br>";
		$dbObj->query($sql);
...
function addItem() {
    ...
    //var p = jQuery('#colorpicker').css('opacity', 0.25);
	//	var selectedX;
		
		jQuery('#' + divIdName+'__label_fg_color')
...
<script type="text/javascript">
  ...
  // function doneme(){ alert('done'); }
		...
		?>
			<div id="item_<?=$field['id']?>" class="lineitem">
				<ul>
					...
					<li style='width: 70px;'><div class='short-field'><input class='colorwell' type="text" style='width: 40px;' id="<?=$field['id']?>__label_fg_color" name="<?=$field['id']?>__label_fg_color" value="<?=$field['label_fg_color']?>" /></div></li>
		...
		echo '</div>';
		?>
		<input type="button" name="nocmd2" value="Add New Attribute" class='btn' onClick="addItem()" />
	</fieldset>
    <br/><br/>
	...
	<input type="button" name="nocmd" class='btn btn-large btn-success' value="Update" onClick="getGroupOrder(this.form)" /> <a href="module/shopping_cart/manage-products/" class='btn btn-large'>Cancel</a>
	</div>
</form>
<?php
