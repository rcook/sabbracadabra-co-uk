...
function load($id) {
	  ...
	  $query = "SELECT * FROM `shopping_cart_products` WHERE id='{$id}'";
	  $result = $dbObj->query($query);
	  $this->attributes = mysql_fetch_array($result);
	  
	  $query = "SELECT * FROM `shopping_cart_optional_product_attributes` WHERE product_id='{$id}'";
	  $result = $dbObj->query($query);
	  while ($attributeItem =  mysql_fetch_array($result)) {
		$attributeID = $attributeItem['id'];
		
		$this->attributes["{$attributeID}"] = $attributeItem;
		$this->attributes["{$attributeID}"]['options'] = array(); 

	  	$query = "SELECT * FROM `shopping_cart_optional_product_attribute_options` WHERE attribute_id='{$attributeID}'";
		//print $query;
	  	$attributeResult = $dbObj->query($query);
		//print mysql_error();
	  	while ($attributeItemOption =  mysql_fetch_array($attributeResult)) {
			$optionID = $attributeItemOption['id'];
			//print "$optionID = ".$attributeItemOption['cost']."<br>";
			$this->attributes["{$attributeID}"]['options']["$optionID"] = $attributeItemOption;  
		}
