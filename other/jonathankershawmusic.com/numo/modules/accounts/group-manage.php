...
<ul class="breadcrumb">
  ...
  <li>&nbsp; <a href="module/accounts/group-create/" style='margin-top: -2px;' class='btn btn-success btn-mini'>Create Group</a></li>
</ul>
<h2>Manage Groups</h2>
<?php
...
echo '<table  class="table table-striped"><tr><th class="highlight_label">Name</th><th>Registration</th><th>Approval</th><th>Activation</th><th>&nbsp</th></tr>';
...
while($row = mysql_fetch_array($results)) {
	...
	if ($access->hasAccess($_GET['m'], "group-edit")) { 
	  echo '<a class="btn btn-primary" href="module/'.$_GET['m'].'/group-edit/?id='.$row['id'].'">Manage</a> ';
	}
	if ($access->hasAccess($_GET['m'], "group-components")) { 
	  echo '<a class="btn" href="module/'.$_GET['m'].'/group-components/?id='.$row['id'].'">Get Component Code</a> ';
	}
	if ($_SESSION['is_admin'] == "1") { 
	  echo '<a class="btn btn-danger" href="module/'.$_GET['m'].'/'.$_GET['i'].'/" onclick="return confirmRemove(\''.$row['id'].'\');">Remove</a>';
	}
	echo '</td></tr>';
